...
public:
  ...
  void load()
  {
    if (obj.vertices().size() != nbVertices || obj.faces().size() != nbFaces)
...
public:
  ...
  void load()
  {
    ...
    if (!file.is_open()) throw std::runtime_error("Can't read off file");
...
auto BM_NormalCalculation = [&](benchmark::State& state, const auto& Inputs) {
  try {
...
void
registerBenchmark(std::string name, BM_NormalCalculationArgs& args)
{
  Object& obj = args.objLoader->obj;
  {
    args.calc = std::make_shared<SequentialNormalCalculation>(obj);
    auto b = benchmark::RegisterBenchmark(
      (name + "/Sequential").c_str(), BM_NormalCalculation, args);
    ...
    for (int nbThreads : { 2, 3, 4, 8 }) {
      {
        ...
        auto b = benchmark::RegisterBenchmark(
          (name + "/OpenMP:" + std::to_string(nbThreads)).c_str(),
          BM_NormalCalculation,
          args);
        benchmarkApplyArgs(b);
        b->UseRealTime();
      }
      {
        args.calc = std::make_shared<PThreadNormalCalculation>(obj, nbThreads);
        auto b = benchmark::RegisterBenchmark(
          (name + "/PThread:" + std::to_string(nbThreads)).c_str(),
          BM_NormalCalculation,
          args);
        benchmarkApplyArgs(b);
        b->UseRealTime();
...
main(int argc, char** argv)
{
  ...
  // Random object
  for (int nbFaces :
       { 100'000, 500'000, 1'000'000, 2'000'000, 5'000'000, 10'000'000 }) {
  ...
  // File object
  for (const std::string& fileName : { "sphere.off",
                                       "can1k5.off",
                                       "bouddha10k.off",
                                       "bouddha500k.off",
                                       "bouddha1m.off" }) {
