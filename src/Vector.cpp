...
int Vector::operator==(const Vector &op) const {
...
Vector &Vector::operator=(const Point &op) {
  x = op.x;
  y = op.y;
  z = op.z;
...
/*! RR 02Juillet2008
 */
Vector Vector::operator+(const Point &_p) const {
...
Vector &Vector::operator*=(const double op) {
  x *= op;
  y *= op;
  z *= op;
  return *this;
...
Vector Vector::normalize(const double lg) {
  double lgtemp;
  if (x != 0 || y != 0 || z != 0) {
