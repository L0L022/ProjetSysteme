...
public:
    ...
    Fun&& fun, Args&&... args)
  {
    ...
    for (auto i = 0; i != n; i++) {
      auto pair =
        measure_execution(std::forward<Fun>(fun), std::forward<Args>(args)...);
  ...
  typename TimeT::rep mean() const { return m_mean; }

  typename TimeT::rep standard_deviation() const { return m_st_dev; }
...
private:
  ...
  {
    auto sum =
      std::accumulate(m_times.begin() + m_throw_away, m_times.end(), 0);
  ...
  void compute_st_dev()
  {
    ...
    std::transform(m_times.begin() + m_throw_away,
                   m_times.end(),
                   diff.begin(),
